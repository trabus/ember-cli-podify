...
module.exports = Task.extend({
  ...
  */
    ...
    var isIgnored = this.getIgnoredFiles().indexOf(fileName[1]) !== -1;
    ...
    if(isFile && !isIgnored && segments.length > 1 && type !== undefined){
  ...
  processFile: function(options, file) {
    ...
    return mkdirp(outputDir)
      .then(function(){
        ...
        this.ui.writeLine(chalk.green('renaming file: ') + '\'' + file.inputPath + '\' to ' + chalk.green('\'' + file.outputPath + '\''));
        if(options.dryRun) {
          return true;
        } else {
      ...
      .catch(function(err){
        // this.ui.writeLine(chalk.red('deleting directory failed: ' + file.inputDir));
        // console.log(err);
        return true;
      }.bind(this));
  },
...
function outputPath(file) {
  ...
  if (file.pod) {
    ...
    if(file.type === 'component') {
      podpath += '/' + inflector.pluralize(file.type);
    }
    if(file.type === 'template' && file.isComponent) {
      subpath = 'components';
    }
